...
readFile("./content/firstOne.txt", "utf8", (err, result) => {
  ...
  if (err) {
    //inside function: checking for an error
  ...
  readFile("./content/secondOne.txt", "utf-8", (err, result) => {
    ...
    writeFile(
      "./content/theSecondNewFile-Async.pdf",
      `This is a new file that I have created as a pdf, you can take this file as a template for my imaginary CV.\nAlsoI will add 100 times 14 with 14 times 7, and the result is: ${
        100 * 14 + 14 * 7
      }.`,
      (err, result) => {
        if (err) {
